...
const Index = () => {
  ...
  return (
    <Layout
      title="RallyRoo - Australia's Home for RC Trucks"
      description="Discover premium RC trucks built for adventure. Off-road, drift, stunt, and DIY kits. Fast Australian shipping, secure checkout."
    >
      {/* Hero Section */}
      <section className="relative overflow-hidden bg-foreground">
        <div className="absolute inset-0">
          <img
            src={heroImage}
            alt="RC truck in Australian outback"
        ...
        <div className="section-container relative py-20 md:py-32">
          <div className="max-w-2xl">
            ...
            <h1 className="mb-6 text-4xl font-bold tracking-tight text-primary-foreground md:text-5xl lg:text-6xl animate-fade-in" style={{ animationDelay: "100ms" }}>
              Adventure Starts with{" "}
              <span className="text-primary">RallyRoo</span>
            </h1>
            <p className="mb-8 text-lg text-primary-foreground/80 md:text-xl animate-fade-in" style={{ animationDelay: "200ms" }}>
      ...
      <section className="bg-muted py-16 md:py-24">
        <div className="section-container">
          ...
          <div className="grid grid-cols-2 gap-4 md:grid-cols-4 md:gap-6">
            {[
              { name: "Off-road", emoji: "🏜️", desc: "Conquer any terrain" },
              { name: "Drift", emoji: "🏎️", desc: "Smooth 360° action" },
              { name: "Stunt", emoji: "🤸", desc: "Flip, roll & tumble" },
              { name: "DIY", emoji: "🔧", desc: "Build your own" },
            ].map((cat) => (
              <Link
                key={cat.name}
                to={`/shop?category=${cat.name}`}
                className="group rounded-xl bg-card p-6 text-center shadow-soft transition-all hover:shadow-elevated hover:-translate-y-1"
              >
